				...
				t.onStart = false;
					...
					/*$.ajax({
						...
						success : function (data) {*/
							//status:获取结果的状态：1-表示成功，2-表示金额不足或次数不足，其他：异常
							//actionStatus:执行指令，1-抽到奖品，2-再来一次(不扣次数),3-谢谢惠顾
							//ran：表示转盘停止的结果，当前指针不动，转盘顺时针转了多少角度，
							//列如转盘分9份，一份40度，ran:40表示转盘在原有的基础上顺时针转了一份的度数，
							//指针指向相邻的下一个奖品
							//onceran:因为刚进入界面时，指针是指向两个奖品中间的分割线的，因此的第一次抽奖时
							//需要添加onceran/2的偏移量使指针停在奖品的中间而不是分割线
							//num:剩余抽奖次数,改为剩余积分
							var data = {'status' : 1, 'actionStatus' : 3, 'ran' : 60, 'onceran' : 60, 'num' : 1}
